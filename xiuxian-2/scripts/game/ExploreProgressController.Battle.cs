...
namespace Xiuxian.Scripts.Game
{
    public partial class ExploreProgressController
    {
        ...
        {
            var resolution = BattleRoundRule.Advance(
                _pendingBattleInputEvents,
                inputEvents,
                _inputsPerBattleRoundRuntime,
                _battleRoundCounter,
                _playerHp,
                _enemyHp,
                _playerAttackPerRoundRuntime,
                _enemyAttackPower,
                _enemyDamageDividerRuntime,
                _enemyMinDamageRuntime);

            int threshold = Mathf.Max(1, _inputsPerBattleRoundRuntime);
            _pendingBattleInputEvents = resolution.PendingInputEvents;
            if (resolution.BattleRoundCounter == _battleRoundCounter)
            {
            ...
            _battleRoundCounter = resolution.BattleRoundCounter;
            _playerHp = resolution.PlayerHp;
            _enemyHp = resolution.EnemyHp;
